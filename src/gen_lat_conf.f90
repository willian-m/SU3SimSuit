...
use heat_bath, only : heat_bath_method,n_slices,heat_bath_method_freezed_slices
...
integer :: nmc,therm,delta
integer :: i,n,m
integer :: rec_step
!==============================

!read input parameters
call read_args(nx,ny,nz,nt,beta,hot_start,therm,n_slices,nmc,delta,rec_step,lattice_file)
...
do n=1,therm
   ...
   print *,"Thermalization", dble(n)*100.0_dp/therm, "% completed."
end do

!Start full updates
do n=1,nmc
   !Performs the update on the blocks, with the time slice frozen
   print*, "Cycle",n,'out of',nmc
   do m=1,delta
      call heat_bath_method_freezed_slices
      print *, "Block update",m,'out of',delta,'in cycle',n,'out of',nmc
      if (mod(m,rec_step) .eq. 0) then
         call write_lattice(n_slices,n,m)
      end if
   end do
   do i=1,rec_step
      call heat_bath_method
...
!==Subroutine to read input arguments
   subroutine read_args(nx,ny,nz,nt,beta,hot_start,therm,n_slices,nmc,delta,rec_step,lattice_file)
   integer, intent(out) :: nx,ny,nz,nt,nmc,rec_step,n_slices,therm,delta
   real(dp),intent(out) :: beta
   logical, intent(out) :: hot_start
   character(len=1024), intent(out) :: lattice_file
   
   integer, parameter :: min_num_par = 11 !Change this if input changes
   character(len=50) :: arg_nx,arg_ny,arg_nz,arg_nt,arg_hot_start,arg_therm,arg_n_slices,arg_nmc,arg_delta,arg_beta,arg_rec_step

   if (COMMAND_ARGUMENT_COUNT() .lt. min_num_par) then
      print*, "It is mandatory to pass at least 10 parameters as input in the format:"
      print*, "nx ny nz nt beta H/C N_Slices NMC Delta recordStep [lattice/Data/FilePath.dat]"
      ...
      print*,"  -Therm: Number of MC updates to thermalize the lattice"
      print*,"  -N_Slices: Number of time slices that will be frozen during one-level execution"
      print*,"  -NMC: a (large) integer to tell how many full updates to perform"
      print*,"  -Delta: Number of updates performed on the block between time-slices"
      ...
      call GET_COMMAND_ARGUMENT(7,arg_therm)
      call GET_COMMAND_ARGUMENT(8,arg_n_slices)
      call GET_COMMAND_ARGUMENT(9,arg_nmc)
      call GET_COMMAND_ARGUMENT(10,arg_delta)
      call GET_COMMAND_ARGUMENT(11,arg_rec_step)
      call GET_COMMAND_ARGUMENT(12,lattice_file)
      read(arg_nx,*) nx
      read(arg_ny,*) ny
      read(arg_nz,*) nz
      read(arg_nt,*) nt
      ...
      read(arg_therm,*) therm
      read(arg_n_slices,*) n_slices
      read(arg_nmc,*) nmc
      read(arg_delta,*) delta
